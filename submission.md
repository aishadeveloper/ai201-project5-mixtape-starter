...
**Regression test:** `tests/regression/test_search_regression.py` — `test_search_sql_produces_one_row_per_song` captures the exact SQL `search_songs()` executes (via a SQLAlchemy event listener), re-executes it raw, and asserts one row per returned song. It failed against the buggy code with `assert 3 == 1` — the same song row appearing three times — and passes after the fix. This white-box design is deliberate: the service's public output hid the duplication, so a behavioral test alone could not have caught the defect before it resurfaced through pagination, `.count()`, or a SQLAlchemy upgrade.
